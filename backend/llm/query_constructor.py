...
class QueryConstructor:
    ...
    def _suggest_filters(self, query: str) -> Dict:
        ...
        for lang in languages:
            if re.search(rf"\b{re.escape(lang)}\b", query):
                filters["language"] = lang
                break

        # Detect code type
        words = set(re.findall(r"\b\w+\b", query))
        if words.intersection({"function", "functions", "def", "method", "methods"}):
            filters["type"] = "function"
        elif words.intersection({"class", "classes", "struct", "structs"}):
            filters["type"] = "class"
        elif words.intersection({"import", "imports", "require"}):
            filters["type"] = "import"
